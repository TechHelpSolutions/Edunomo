...
import { Bell, User, Compass, LogOut, ChevronRight, LayoutDashboard, Sparkles, Menu, X } from 'lucide-react';
            ...
            {!isAuthenticated ? (
              ...
              <>
                ...
                <div className="hidden lg:flex items-center gap-2 sm:gap-3">
                  <Link
                    to="/partner/login"
                    className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:text-[#0D2A68] hover:bg-slate-100 rounded-lg transition-colors"
                  >
                ...
                <button
                  ...
                  className="lg:hidden min-w-[44px] min-h-[44px] flex items-center justify-center rounded-xl text-slate-700 hover:text-[#0D2A68] hover:bg-slate-100 active:bg-slate-200 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0D2A68]"
                  aria-label={isMobileMenuOpen ? 'Close navigation menu' : 'Open main navigation menu'}
                  aria-expanded={isMobileMenuOpen}
                  aria-controls="mobile-nav-drawer"
                >
                  {isMobileMenuOpen ? <X className="w-6 h-6 text-[#0D2A68]" /> : <Menu className="w-6 h-6 text-slate-800" />}
                </button>
              </>
            ) : (
              /* LOGGED IN STATE */
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                {/* Mobile Hamburger Menu Button (Authenticated) */}
                <button
                  type="button"
                  onClick={(e) => {
                    e.preventDefault();
